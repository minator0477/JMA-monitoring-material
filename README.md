...
- `scripts/monitor.py` — 監視・ダウンロードを行う本体スクリプト。新着があれば `state/new_entries.json` に書き出す。
...
通知にはGitHub上のPDFへのリンク(例: `https://github.com/<owner>/<repo>/blob/main/downloads/<id>/xxx.pdf`)を掲載します。このリンクはpush後でないと閲覧できないため、`notify.py` は必ずコミット・push後に実行してください(`.github/workflows/monitor.yml` はその順序で組んであります)。
...
6. push後に `notify.py` が `state/new_entries.json` を読み、GitHub上のPDFへのリンクをDiscordに通知
